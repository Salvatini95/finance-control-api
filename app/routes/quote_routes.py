...
def get_quotes():
    ...
    if user.company_id:
        quotes = Quote.query.filter_by(company_id=user.company_id).order_by(Quote.id.desc()).all()
    else:
        quotes = Quote.query.filter_by(user_id=user.id).order_by(Quote.id.desc()).all()
...
def create_quote():
    ...
    if user.company_id:
        last = Quote.query.filter_by(company_id=user.company_id).order_by(Quote.id.desc()).first()
    else:
        last = Quote.query.filter_by(user_id=user.id).order_by(Quote.id.desc()).first()
