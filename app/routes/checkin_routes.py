...
"""
Rotas de Check-in — apenas HTTP.
Toda lógica de negócio está em CheckinService.
"""
...
def get_qr_token():
    ...
    token = CheckinService.gerar_url_qr_universal()
    return jsonify({"qr_token": token}), 200


# ── POST /api/checkin/<client_id>/start ───────────────────────────────────────
@checkin_bp.route("/checkin/<int:client_id>/start", methods=["POST"])
@jwt_required()
def checkin_start(client_id: int):
    user = _get_user(get_jwt_identity())
...
@jwt_required()
def checkin_finish(checkin_id: int):
    user = _get_user(get_jwt_identity())
    data = request.get_json() or {}
